...
const CommandsContainer = require('../models/CommandsContainer');
	...
	if (!message.author.bot)
	{
		...
		.then(async function (prefix)
		{
			...
			if (prefix && (message.content.startsWith(prefix) || botWasPinged(message)))
			{
				...
				// Run the command.
				await runCommands(bot, command, message.author, message.author.id, message.channel.id, message, args, prefix);
			}
		})
		.catch(function (err)
		{
...
async function runCommands(bot, userCommand, user, userId, channelId, message, args, prefix)
{
